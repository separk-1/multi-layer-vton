...
class MatrixVisualizer:
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        if matrix is not None:
            ...
            # 🔥 여기 핵심: matrix_vis를 (w, h) 크기로 resize
            matrix_vis_resized = cv2.resize(matrix_vis, (w_mask, h_mask), interpolation=cv2.INTER_LINEAR)

            # 🔥 blending도 mask 크기 기준
            image_target_bgr[y : y + h_mask, x : x + w_mask, :] = (
                image_target_bgr[y : y + h_mask, x : x + w_mask, :] * (1.0 - self.alpha)
                + matrix_vis_resized * self.alpha
            )
